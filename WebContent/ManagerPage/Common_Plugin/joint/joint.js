...
(function(root, factory) {
    ...
    VElement.prototype = {
		...
		box = { x: box.x | 0, y: box.y | 0, width: box.width | 0, height: box.height | 0};
            ...
            var matrix = this.node.getCTM(target || this.node.ownerSVGElement);
		...
		var globalToLocalMatrix = this.node.getCTM(svg).inverse();
        ...
        translateAndAutoOrient: function(position, reference, target) {
            ...
            var ctm = this.node.getCTM(target);
...
(function(root, factory) {
    ...
    rect.prototype = {
	    ...
	    for (var i = sides.length - 1; i >= 0; --i){
	    	...
	    	if(center.y<result.y){
	    		result.x=center.x;
	    		result.y=center.y+ this.height/2;
	    	}

	    	if(center.y>result.y){
	    		
	    		if(Math.abs(center.x-p.x)<this.width/2){
	    			
	    		}else if(center.x<result.x){
	    			result.x=center.x+this.width/2;
	    			result.y=center.y;
	    		}else if(center.x>result.x){
	    			result.x=center.x-this.width/2;
	    			result.y=center.y;
...
joint.connectors.normal = function(sourcePoint, targetPoint, vertices) {
    ...
    d.push(sourcePoint.x,targetPoint.y);
